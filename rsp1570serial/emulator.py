...
SOURCE_ATTRIB_MAP = {
    ' CD': SourceAttribs(
        ...
        ['--cd', '--alias_cd'], 'alias_cd'),
    'TUNER': SourceAttribs(
        "STEREO          44.1K",
        ['A', 'Standby LED', 'SW', 'FR', 'FL'],
        ['--tuner', '--alias_tuner'], 'alias_tuner'),
    'TAPE': SourceAttribs(
        "STEREO          44.1K", 
        ['A', 'Standby LED', 'SW', 'FR', 'FL'], 
        ['--tape', '--alias_tape'], 'alias_tape'),
    'VIDEO 1': SourceAttribs(
        "DOLBY PL\x19 C     48K  ", 
        ['II', 'HDMI', 'Pro Logic', 'Standby LED', 'SW', 'SR', 'SL', 'FR', 'C', 'FL'],
        ['--video_1', '--alias_video_1'], 'alias_video_1'),
    'VIDEO 2': SourceAttribs(
        "DOLBY PL\x19 M     48K  ",
        ['II', 'HDMI', 'Pro Logic', 'Standby LED', 'SW', 'SR', 'SL', 'FR', 'C', 'FL'],
        ['--video_2', '--alias_video_2'], 'alias_video_2'),
    'VIDEO 3': SourceAttribs(
        "DOLBY DIGITAL   48K  ", 
        ['HDMI', 'Optical', '1', 'Dolby Digital', 'Standby LED', 'SW', 'SR', 'SL', 'FR', 'C', 'FL'],
        ['--video_3', '--alias_video_3'], 'alias_video_3'),
    'VIDEO 4': SourceAttribs(
        "5CH STEREO      48K  ", 
        ['A', 'Standby LED', 'SW', 'SR', 'SL', 'FR', 'C', 'FL'],
        ['--video_4', '--alias_video_4'], 'alias_video_4'),
    'VIDEO 5': SourceAttribs(
        "PCM 2CH         48K  ", 
        ['A', 'Standby LED', 'SW', 'FR', 'FL'],
        ['--video_5', '--alias_video_5'], 'alias_video_5'),
    'MULTI': SourceAttribs(
        "BYPASS          48K  ", 
        ['Standby LED', 'CBL', 'CBR', 'SW', 'SR', 'SL', 'FR', 'C', 'FL'],
        ['--multi', '--alias_multi'], 'alias_multi'),
}
...
class Blinker:
    ...
    def start(self):
        ...
        self._blink_task = loop.create_task(self.blinker())
...
class RotelRSP1570Emulator:
    ...
    async def set_volume(self, level):
        if self._is_muted:
            # NOT a call to mute_off()
            self._is_muted = False
        if level < MIN_VOLUME or level > MAX_VOLUME: return
        self._volume = level
        await self.write_feedback_message()
    ...
    async def set_source(self, source):
        if source not in SOURCE_ATTRIB_MAP: return
        self._source = source
        await self.write_feedback_message()
...
class CommandHandler:
    ...
    async def apply_volume_direct_command(self, command):
        if command.message_type == MSGTYPE_VOLUME_DIRECT_COMMANDS:
            logging.info("Processing volume direct command.  Setting Volume to %d", command.key[0])
            await self._device.set_volume(command.key[0])
        else:
            logging.info("Unsupported volume direct command: '%r', Key: '%r' ignored", command.message_type, command.key)
        
    async def apply_simple_command(self, command):
        command_code = self._command_code_lookup.get(bytes((command.message_type, command.key[0])), None)
        logging.info("Message Type: '%r', Key: '%r', Code: '%s'", command.message_type, command.key, command_code)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-p', '--port', type=int, default=50001, help="port to serve on")
    parser.add_argument('-o', '--is_on', action='store_true', help="emulator starts up in the on state")
    for name, attribs in SOURCE_ATTRIB_MAP.items():
        parser.add_argument(*attribs.alias_args, type=str, dest=attribs.argparse_dest, help="alias for '{}' input".format(name))
    ...
    run_server(args.port, aliases, args.is_on)